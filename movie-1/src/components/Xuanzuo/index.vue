    ...
    <div v-height="50">
      ...
      <div class="xzOk">
        <div class="yx" v-if="isShow">
          ...
          <button class="but" @click="queren">{{gpprice}}元 确认选座</button>
        </div>
        <div class="wx" v-else>
          <div>
      ...
      <van-dialog class="tishi" v-model="isShowTiShi" show-cancel-button :beforeClose="beforeClose">
        <p>{{xinxi}}</p>
      </van-dialog>
    </div>
  </div>
...
export default {
  ...
  methods: {
    djxz(pai, zuo, zt, price) {
      //参数1.第几排 2.第几个座位 3.是否为已售出 4.每个座位单价
      if (!zt) {
        //判断是否为红色
        if (
          this.yixuanzuowei.length < 4 ||
    ...
    beforeClose(action, done) {
      if (action === "confirm") {
        if (this.xinxi == "请登录") {
          this.$router.push({ path: "/login",query:{user:0}});
        }
        done();
